...
**[アプリをブラウザで開く](https://weather-checker-woad.vercel.app/)**
...
- **OpenWeatherMap API キー**: [公式サイト](https://openweathermap.org/)で取得が必要です。
- **ブラウザの位置情報取得許可**: 天気取得のために必要です。

### 開発手法別の必要環境

- **A. Dev Containers を使用する場合**
  - Docker Desktop
  - Visual Studio Code + Dev Containers 拡張機能
- **B. ローカル環境で直接構築する場合**
  - Node.js (推奨: v20.0.0 以上)
  - npm (推奨: v10.0.0 以上)
...
### 2.A 🚀 推奨：Dev Containers を使用する場合

**VS Code** と **Docker** を使用して、最短で開発環境を構築します。

1. **VS Code** でプロジェクトフォルダを開きます。
2. コマンドパレット（**`Ctrl` + `Shift` + `P`**）を開き、以下のコマンドを入力・選択します。
   > `Dev Containers: Reopen in Container`
3. コンテナの起動後、以下の環境が**自動的にセットアップ**されます：
   - **Node.js 20** 開発環境の構築
   - **推奨拡張機能の導入**（ESLint, Prettier 等）
